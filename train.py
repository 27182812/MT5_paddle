...
from paddlenlp.transformers.mt5 import MT5ForConditionalGeneration, T5Tokenizer
from tqdm import tqdm

from args import parse_args
from data import get_dev_dataloader_xnli, get_train_dataloader_xnli, get_test_dataloader_xnli
...
def main(args):
    ...
    # get dataloader
    train_dataloader = get_train_dataloader_xnli(tokenizer, args)
    dev_dataloader = get_dev_dataloader_xnli(tokenizer, args)
    test_dataloader = get_test_dataloader_xnli(tokenizer, args)
            ...
            if (
                ...
                if args.save_steps > 0 and global_steps % args.save_steps == 0:
                    ...
                    for k, v in test_results.items():
                        writer.add_scalar(f"test/{k}", v, global_steps)
                        logger.info(f"  {k} = {v}")
                        if int(v) >= 88:
                            output_dir = os.path.join(args.output_dir, f"step-{global_steps}")
                            os.makedirs(output_dir, exist_ok=True)
